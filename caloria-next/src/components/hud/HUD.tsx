  ...
  return (
    <>
      ...
      <div className="fixed top-4 left-4 z-40">
        <div className="sf-panel px-5 py-4 holo-flicker" style={{ minWidth: 260 }}>
          <div className="flex items-center gap-2.5 mb-3">
            <div className="font-hud text-sm" style={{ color: 'rgba(0,212,255,0.6)' }}>LV</div>
            <div className="font-hud text-4xl text-glow leading-none">{playerLevel}</div>
            <div
              className="font-hud px-2 py-1 ml-1"
              style={{ fontSize: '0.85rem', border: '1px solid rgba(0,212,255,0.3)', color: 'rgba(0,212,255,0.7)' }}
            >
              {getTier(playerLevel)}
            </div>
          </div>
          ...
          <div className="mb-2">
            <div className="flex justify-between mb-1.5" style={{ color: 'rgba(0,212,255,0.6)', fontSize: '0.9rem' }}>
              <span className="font-hud">EXP</span>
              <span>{playerExp} / {expToNextLevel}</span>
            </div>
            <div className="h-2.5 w-full rounded-none" style={{ background: 'rgba(0,212,255,0.1)', border: '1px solid rgba(0,212,255,0.2)' }}>
          ...
          <div className="flex items-center justify-between mt-2.5">
            <div className="flex items-center gap-2">
              <div className="font-hud text-sm" style={{ color: 'rgba(0,212,255,0.5)' }}>TIME</div>
              <div className="font-hud text-base" style={{ color: '#00d4ff' }}>{fmtTime(exerciseSeconds)}</div>
            </div>
            <div className="flex items-center gap-1.5">
              <span className="font-hud text-base" style={{ color: '#aa44ff' }}>◈</span>
              <span className="font-hud text-base" style={{ color: '#cc88ff' }}>{crystals.toLocaleString()}</span>
            </div>
          </div>

          {boosterActive && (
            <div className="mt-2.5 px-2.5 py-1.5 text-sm font-hud pulse"
      ...
      {leadChar && (
        <div className="fixed top-4 right-4 z-40">
          <button
            className="sf-panel px-5 py-3.5 text-right holo-flicker w-full"
            style={{ cursor: onOpenSwap ? 'pointer' : 'default', background: 'rgba(0,10,30,0.85)' }}
            onClick={onOpenSwap}
          >
            <div className="font-hud text-base leading-none mb-1" style={{ color: leadChar.color }}>
              {leadChar.name}
            </div>
            <div
      ...
      {activeChar && (
        <div
          ...
          style={{ bottom: 20, left: '50%', transform: 'translateX(-50%)', pointerEvents: 'none' }}
        >
          {(() => {
            ...
            return (
              <div className="sf-panel" style={{ padding: '12px 26px', minWidth: 320 }}>
                <div className="flex items-center justify-between mb-2">
                  <span className="font-hud text-base" style={{ color: activeChar.color }}>{activeChar.name}</span>
                  <span className="font-hud" style={{ fontSize: '0.9rem', color: 'rgba(224,240,255,0.45)' }}>
                    {hp.cur} / {maxHp}
                  </span>
                </div>
                <div style={{ height: 10, background: 'rgba(0,212,255,0.12)', border: '1px solid rgba(0,212,255,0.2)' }}>
                  <div style={{
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    {bonusChips.map(({ label, value }) => (
                      <span key={label} className="font-hud"
                        style={{ fontSize: '0.68rem', padding: '3px 8px', background: 'rgba(255,215,0,0.1)', border: '1px solid rgba(255,215,0,0.3)', color: '#ffd700' }}>
                        {label} {value}
                      </span>
                    ))}
                  </div>
